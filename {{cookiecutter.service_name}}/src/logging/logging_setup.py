...
import atexit, json as _json, logging, os, sys, threading, traceback, uuid
...
FILE_TEXT_FORMAT = "{time:YYYY-MM-DD HH:mm:ss.SSS} | {level: <8} | {name}:{function}:{line} | {message}"


def _flat_json_line(record: Any, service: str, environment: str) -> str:
    out: dict[str, Any] = {
        "timestamp": record["time"].isoformat(), "level": record["level"].name,
        "message": record["message"], "service": service, "environment": environment,
        "logger": record["name"], "function": record["function"], "line": record["line"],
    }
...
def setup_logging(settings: Optional[Any] = None) -> None:
    ...
    with _logging_lock:
        ...
        if use_json:
            logger.add(
                _make_flat_json_sink(_SERVICE_NAME, environment,
                                     lambda l: open(log_file, "a").write(l)),
            ...
            logger.add(log_file, rotation="00:00", retention="30 days", compression="gz",
                       level=log_level, format=FILE_TEXT_FORMAT, enqueue=True)
            logger.add(ResilientSink(sys.stdout).write, level=log_level, format=TEXT_FORMAT,
                       backtrace=True, diagnose=is_dev, enqueue=True, colorize=is_dev)

        logger.configure(patcher=_context_patcher,
                         extra={"service": _SERVICE_NAME, "environment": environment,
